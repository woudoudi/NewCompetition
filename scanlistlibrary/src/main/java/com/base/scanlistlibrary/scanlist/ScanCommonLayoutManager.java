 ...
 * @author zhaolong
 */
public class ScanCommonLayoutManager extends LinearLayoutManager {
    ...
    public ScanCommonLayoutManager(Context context, boolean isPaging) {
        super(context);
        init(isPaging);
    }
...
//                int position = findFirstCompletelyVisibleItemPosition();
                int positionIdle = findFirstVisibleItemPosition();
...
//                }
                ...
                float scale = getVisibleScaleByPosition(positionIdle);
                if(positionIdle==0){
                    float tempScale = getVisibleScaleByPosition(positionIdle+1);
                    if(tempScale>0.5){
                        positionIdle+=1;
                    }
                } else {
    ...
    private float getVisibleScaleByPosition(int positionIdle){
        View firstView = findViewByPosition(positionIdle);
        int itemHeight;
        if(firstView!=null){
            Rect rect=new Rect();
