...
public class GlideCatchUtil {
    ...
    public String getCacheSize(Context context) {
        try {
            return getFormatSize(getFolderSize(new File(context.getCacheDir() + "/" + DEFAULT_DISK_CACHE_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    public boolean cleanCatchDisk(Context context) {
        return deleteFolderFile(context.getCacheDir() + "/" + DEFAULT_DISK_CACHE_DIR, true);
    }
