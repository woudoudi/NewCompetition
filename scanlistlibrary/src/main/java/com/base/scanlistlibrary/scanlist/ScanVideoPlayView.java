...
public class ScanVideoPlayView<A> extends FrameLayout {
    ...
    public void initPlayListView(ScanBaseRecyclerViewAdapter<A> adapter, int emptyLayoutId, boolean isPaging) {
        initPlayListView(adapter,1,emptyLayoutId,isPaging,null);
    }

    public void initPlayListView(ScanBaseRecyclerViewAdapter<A> adapter, int colum, int emptyLayoutId, boolean isPaging) {
        initPlayListView(adapter, colum, emptyLayoutId, null, isPaging,null);
    }

    public void initPlayListView(ScanBaseRecyclerViewAdapter<A> adapter, int colum, int emptyLayoutId, boolean isPaging,GridLayoutManager.SpanSizeLookup spanSizeLookup) {
        initPlayListView(adapter, colum, emptyLayoutId, null, isPaging,spanSizeLookup);
    }

    public void initPlayListView(ScanBaseRecyclerViewAdapter<A> adapter, int colum, int emptyLayoutId, String emptyMsg, boolean isPaging,GridLayoutManager.SpanSizeLookup spanSizeLookup) {
        initPlayListView(adapter, colum, emptyLayoutId, emptyMsg, false,isPaging,spanSizeLookup);
    }
     ...
     */
    public void initPlayListView(ScanBaseRecyclerViewAdapter<A> adapter, int colum, int emptyLayoutId, String emptyMsg,boolean isStager, boolean isPaging,GridLayoutManager.SpanSizeLookup spanSizeLookup) {
        mScanVideoListView = new ScanVideoListView(mContext, colum, emptyLayoutId, emptyMsg,isStager, isPaging,spanSizeLookup);
...
//        mScanVideoListView.setPlayerCount(3);
    ...
    public void setOnLoadMoreEnable(boolean b) {
        if (mScanVideoListView != null) {
            mScanVideoListView.setOnLoadMoreEnable(b);
        }
    }
    ...
    /**
     * @return 返回当前的加载模式，上拉刷新还是下拉刷新
     */
    public int getCurrentLoadingStatus() {
        return mScanVideoListView.getCurrentLoadingStatus();
    }

    public void setCurrentLoadingStatus(int currentLoadingStatus) {
         mScanVideoListView.setCurrentLoadingStatus(currentLoadingStatus);
    }

    public void setNestedScrollingEnabled(boolean enabled){
        mScanVideoListView.setNestedScrollingEnabled(enabled);
    }

    public ScanVideoListView getmScanVideoListView() {
        return mScanVideoListView;
    }
}
